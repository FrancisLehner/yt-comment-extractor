    ...
    for root, dirs, files in os.walk(file_directory):
        for file in files:
            if file.endswith(".csv"):
                ...
                dataframes[df_name] = pd.concat([chunk], axis=0)
                del chunk_list
...
def concat():
    ...
    concat_df = pd.concat(all_dataframes.values(), ignore_index=True)

    concat_df = concat_df.reset_index()
    concat_df.to_csv(f"{directory_ext}/comments_concat_df.csv")
    # print(concat_df)
...
def common_words():
    ...
    common_word_df.to_csv(f'{directory_ext}/common_words_Jan23_Apr24.csv')
