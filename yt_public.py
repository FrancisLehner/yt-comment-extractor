...
def automated_comment_fetch(ChannelIdentifier, API_KEY):
    ...
    for video_id in videoMetadata:
        ...
        if videoId not in processed_video_ids:
            ...
            comments_list, error = comment_threads(videoId, to_csv=True)
            ...
            if error:
                ...
                if error.resp.status == 403:
                    ...
                    if error_reason == 'commentsDisabled':
                        print(f'Comments are disabled for this video. Ignoring comment scraping for {videoId}')
                        continue
                    else:
                        print(f"Can't extract comments: {error}")
                        continue
                else:
                    print(error)
            ...
            destination_path = os.path.join(f"{directory}/extracted_comments/{destination_folder}", f"comments_{videoId}_{today}.csv")
            if os.path.exists(destination_path):
                os.remove(destination_path)
            shutil.copy(f"{directory}/comments_{videoId}_{today}.csv", destination_path)
